...
public class AdvancedToolModel implements IModel
	{
	...
	public AdvancedToolModel(ImmutableList<ResourceLocation> defaultTextures, List<MaterialModel> parts, List<MaterialModel> brokenPartBlocks, Float[] layerRotations, ModifierModel modifiers, ImmutableMap<TransformType, TRSRTransformation> transforms, ImmutableList<ToolModelOverride> overrides, AmmoPosition ammoPosition)
		{
	...
	public IBakedModel bake(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter)
		...
		try
			{
			Reader reader = ModelHelper.getReaderForResource(new ResourceLocation("block/cube_all"));
			ModelBlock model = ModelBlock.deserialize(reader);
			base = new ItemLayerModel(textures, model.createOverrides()).bake(state, format, bakedTextureGetter);
			}
		catch (IOException e)
			{
			e.printStackTrace();
			base = new ItemLayerModel(textures).bake(state, format, bakedTextureGetter);
			}
	...
	private IModelState getStateForPart(int i, IModelState originalState)
		{
		if (layerRotations.length > i) { return new ModelStateComposition(originalState, TRSRTransformation.blockCenterToCorner(new TRSRTransformation(null, TRSRTransformation.quatFromXYZ(0, 0, (float) (layerRotations[i] * Math.PI / 180)), null, null))); }
